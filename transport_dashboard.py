...
<h1 class="centered-title">Transport Energy Results</h1>
"""
...
eu27_data = transport_data[transport_data['Country'] == 'EU27_2020']
...
# Consistent ordering
main_categories = ['Road', 'Aviation', 'Rail']
colors_main = custom_blues[:len(main_categories)]

fig_main = px.bar(
    pivot_main,
    x=pivot_main.index,
    y=main_categories,
    title="EU27 Energy Demand by Transport Mode (Road, Aviation, Rail)",
    labels={'value': 'Energy Demand (EJ)', 'Year': 'Year'},
...
data_without_eu27 = transport_data[transport_data['Country'] != 'EU27_2020']
...
filtered_data = transport_data[
    ...
    (transport_data['Country'] != 'EU27_2020')  # Exclude total of eu27
]
import pycountry
...
# -------------------- Fuel distribution for 4 wheelers ----------------------------------------

fuel_data = pd.read_csv(r"C:\Users\mar.eco\OneDrive - CBS - Copenhagen Business School\Desktop\REMIND\Results_REMIND_4w_yearly.csv")
fuel_2020 = fuel_data[fuel_data['Year'] == 2020].copy()
fuel_2020['Fuel'] = fuel_2020['Category'].str.split('|').str[-1]

fuel_distribution = fuel_2020.groupby('Fuel')['Value'].sum().reset_index()

st.write(fuel_distribution)  
st.write(fuel_distribution.dtypes)  
...
fig_fuel_bar = px.bar(
    ...
    x='Fuel',
    y='Value',
    title="Fuel Distribution for Passenger Cars (4-Wheelers) in 2020",
    text_auto='.2s'
)
st.plotly_chart(fig_fuel_bar)
