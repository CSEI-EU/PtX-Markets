...
for scenario in scenarios:
    for region in regions:
        for sector in sectors:
            if sector == 'Transport': # only to be consistent with transport = subsectors
...
fig = px.line(global_demand, x='Year', y='Value', color='Scenario', markers=True,
...
highest_value_2025 = demand_2025[demand_2025['Sector'] == highest_sector_2050]['Value'].sum()
percent_increase_sector = ((highest_value_2050 - highest_value_2025) / highest_value_2025) * 100
...
with key_num:
    ...
    st.info(f"The **{highest_sector_2050}** sector is predicted to have the highest energy demand of **{highest_value_2050} GWh** in 2050, compared to **{highest_value_2025} GWh** in 2025, a **{percent_increase_sector:.1f}%** increase.")
...
for tab, scenario in zip(tabs, scenarios):
    with tab:
        ...
        fig.update_layout(template='plotly_white', yaxis=dict(range=[0, 6000]))
        st.plotly_chart(fig, use_container_width=True)
        ...
        if scenario == "Hydrogen":
            ...
            with col1:
                st.plotly_chart(pie_chart_2025, use_container_width=True)
            with col2:
                st.plotly_chart(pie_chart_2050, use_container_width=True)
