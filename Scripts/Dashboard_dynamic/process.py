...
- load_scenario_outputs: Load scenario excel files from Outputs/Scenarios for a given scenario name.
...
@st.cache_data
def load_scenario_outputs(scenario: str, scenario_folder_path: str) -> pd.DataFrame:
    if not os.path.exists(scenario_folder_path):
        return pd.DataFrame()

    suffix = f"_{scenario}.xlsx"
    files = [f for f in os.listdir(scenario_folder_path) if f.endswith(suffix)]
    ...
    all_data = []
    for file in files:
        # PtX_demand_{CC}_{scenario}.xlsx → strip suffix, take last token
        country_code = file.replace(suffix, "").split("_")[-1]
        file_path = os.path.join(scenario_folder_path, file)

        df = pd.read_excel(file_path)
        sector_cols = [c for c in df.columns if c not in ['FuelGroup', 'Year']]
        df_long = df.melt(
            id_vars=['FuelGroup', 'Year'],
